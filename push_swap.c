...
/*   Updated: 2022/05/28 03:05:43 by jucheval         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	// ft_rotate_ra_rb(data.stack_a, data.len_a, _STACK_A);				// shift up all elements of A stack. the first element becomes the last one
	// ft_rotate_ra_rb(data.stack_b, data.len_b, _STACK_B);				// shift up all elements of B stack. the first element becomes the last one
	// ft_print_stack(data.stack_a, data.len_a);							// check the result with the A stack
	// ft_print_stack(data.stack_b, data.len_b);							// check the result with the B stack
	...
	// ft_reverse_rotate_rra_rrb(data.stack_a, data.len_a, _STACK_A);		// shift down all elements of A stack. the last element becomes the first
	// ft_reverse_rotate_rra_rrb(data.stack_b, data.len_b, _STACK_B);		// shift down all elements of A stack. the last element becomes the first
	// ft_print_stack(data.stack_a, data.len_a);							// check the result with the A stack
	// ft_print_stack(data.stack_b, data.len_b);							// check the result with the B stack
